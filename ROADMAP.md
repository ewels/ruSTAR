...
| [16](docs/phase16_algorithm.md) | Algorithm Parity | ✅* | 269 | 99.5% pos, 2.0% splice, 99.6% MAPQ, 77 actionable, 7 STAR-only |
...
**Summary**: Bin-based windowing, pre-DP seed extension, MMP SA range narrowing, multi-transcript DP, recursive combinatorial stitcher, STAR-faithful scoring (scoreStitchSJshift removed), sparse bidirectional seed search with Nstart +1 fix, WALrec persistent threshold, post-jR shared base scoring, hierarchical SAindex lookup, nWA reset + overlap detection, coverage filter removal + Lread-1 filter fix (99.5% pos adjusted, 98.7% CIGAR, 2.0% splice, 99.6% MAPQ agree, 77 actionable), mate rescue.

**Remaining disagreements (10k SE yeast, 142 total, 77 actionable):**

| Root Cause | Count | Status |
|-----------|-------|--------|
| Diff-chr multi-mapper ties | 99 | Unavoidable (random seed) |
| Same-chr repeat tie-breaking | ~41 | Mix of unavoidable + algorithm differences |
| Missed splices (STAR finds, ruSTAR doesn't) | ~15 | Short exon + large intron |
| MAPQ inflation (ruSTAR=255, STAR=multi) | 30 | Related to multi-loci discovery |
| Mapping-only differences (one maps, other doesn't) | ~34 | 7 STAR-only (2 splice + 5 edge) + 27 ruSTAR-only |
